...
| **`GET /model-pipeline/config`** (**D7b**) | Read-only JSON view of effective pipeline settings + purpose registry status — **does not** return **`GRAPHCLERK_MODEL_PIPELINE_API_KEY`** or raw base URL; **does not** call Ollama or list remote models. Use for operator/debug alignment with env. |
