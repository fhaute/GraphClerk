...
**Status:** **Skeleton (Track F Slice F1).** This folder is the **living** home for “how to run GraphClerk in your stack” guidance. The entry below and [`GRAPHCLERK_PIPELINE_GUIDE.md`](GRAPHCLERK_PIPELINE_GUIDE.md) are **structured and truthful** for the current codebase; **curl/Python cookbooks**, diagrams, and production runbooks are **planned** (Tracks **F2–F5** and later slices). Examples elsewhere may be **partial** until those slices land.
...
2. Skim **[`docs/api/API_OVERVIEW.md`](../api/API_OVERVIEW.md)** — HTTP surface in one table.
3. Run or read **[`docs/demo/PHASE_6_DEMO_CORPUS.md`](../demo/PHASE_6_DEMO_CORPUS.md)** — script-driven demo corpus and **manual vector indexing** notes (including Qdrant **dimension mismatch** — [`docs/governance/TESTING_RULES.md`](../governance/TESTING_RULES.md)).
4. Use **[`docs/release/RELEASE_CHECKLIST.md`](../release/RELEASE_CHECKLIST.md)** before release-style handoffs.
...
| Document | Purpose |
|----------|---------|
| [`GRAPHCLERK_PIPELINE_GUIDE.md`](GRAPHCLERK_PIPELINE_GUIDE.md) | Main pipeline / integration story (skeleton). |
...
- **[`docs/plans/phase_1_8_completion_program.md`](../plans/phase_1_8_completion_program.md)** — Tracks **A–H**; **Track F** = this onboarding series (**F1** skeleton; **F2–F5** open).
...
- **Onboarding is not finished** — F1 establishes **place + structure**; cookbooks and diagrams are **future work**.
...
|-------|----------------------------------|
| **F2** | Minimal “feed content” guide |
| **F3** | Full pipeline narrative + architecture diagram |
| **F4** | Failure modes + Qdrant operator detail |
| **F5** | Examples cookbook (curl / Python) |

---

## Open decisions (do not block F1)
