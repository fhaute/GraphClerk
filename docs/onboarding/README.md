...
**Status:** **Living docs (Track F: F1 + F2 + F3).** [`GRAPHCLERK_PIPELINE_GUIDE.md`](GRAPHCLERK_PIPELINE_GUIDE.md) is the **conceptual** overview; [`FEED_CONTENT_MINIMAL_GUIDE.md`](FEED_CONTENT_MINIMAL_GUIDE.md) is the **minimal hands-on** path; [`GRAPHCLERK_ARCHITECTURE.md`](GRAPHCLERK_ARCHITECTURE.md) is the **as-built architecture** (Mermaid + stores + API/UI maps). **F4–F5** (expanded failure/Qdrant narrative, curl/Python cookbook) and production runbooks remain **planned**. **Onboarding is not complete.**
...
4. Skim **[`docs/api/API_OVERVIEW.md`](../api/API_OVERVIEW.md)** — HTTP surface in one table.
5. Run or read **[`docs/demo/PHASE_6_DEMO_CORPUS.md`](../demo/PHASE_6_DEMO_CORPUS.md)** — script-driven demo corpus and **manual vector indexing** notes (including Qdrant **dimension mismatch** — [`docs/governance/TESTING_RULES.md`](../governance/TESTING_RULES.md)).
6. Use **[`docs/release/RELEASE_CHECKLIST.md`](../release/RELEASE_CHECKLIST.md)** before release-style handoffs.
...
- **[`docs/plans/phase_1_8_completion_program.md`](../plans/phase_1_8_completion_program.md)** — Tracks **A–H**; **Track F** = this onboarding series (**F1–F3** shipped; **F4–F5** open).
...
- **Onboarding is not finished** — F3 adds **architecture** Mermaid + narrative; **F4–F5** (expanded failure/Qdrant operator doc, curl/Python cookbook) are **future work**.
...
## Open decisions (do not block F1 / F2 / F3)
