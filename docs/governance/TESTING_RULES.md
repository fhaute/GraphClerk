...
- **`ollama` (Track D Slice D3):** [`OllamaModelPipelineAdapter`](../../backend/app/services/model_pipeline_ollama_adapter.py) calls Ollama **`POST /api/generate`** (stdlib **`urllib`**, **`stream: false`**, **`format: json`**). Registry requires non-empty **`GRAPHCLERK_MODEL_PIPELINE_BASE_URL`** and **`GRAPHCLERK_MODEL_PIPELINE_MODEL`** — otherwise **`model_pipeline_ollama_misconfigured`** (**no** silent fallback). Single env model is **not** the final **D4** per-purpose architecture ([**D2.5**](../decisions/phase_8_model_pipeline_completion_decisions.md)).
