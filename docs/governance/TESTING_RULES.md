...
- **Operator UI visibility (Track D Slice D7a):** Read-only **Phase 8** notes + conditional **`graphclerk_model_pipeline`** readout in **`frontend/src/components/ArtifactsExplorer.tsx`** (and a small static note in **`EvaluationDashboard.tsx`**) — **no** writable selector, **no** admin config persistence, **no** new backend endpoints or ingest/retrieve behavior. Verify frontend with **`npm run build`** from **`frontend/`** (no backend tests added for this slice).
