...
| `GRAPHCLERK_MODEL_PIPELINE_EVIDENCE_ENRICHER_ENABLED` | **`false`** \| **`true`** | **`false`** |
| `GRAPHCLERK_MODEL_PIPELINE_EVIDENCE_ENRICHER_MODEL` | non-empty string when enricher enabled | unset |
| `GRAPHCLERK_MODEL_PIPELINE_EVIDENCE_ENRICHER_TIMEOUT_SECONDS` | optional float **`0 < value <= 300`** | unset (falls back to **`GRAPHCLERK_MODEL_PIPELINE_TIMEOUT_SECONDS`**) |

- **Default:** **`not_configured`** — [`build_model_pipeline_adapter`](../../backend/app/services/model_pipeline_registry.py) returns **`NotConfiguredModelPipelineAdapter`**. **Default** ingest path makes **no** model pipeline HTTP calls. **`POST /answer`** is **not** implemented; default **`pytest`** uses **mocked** / stub adapters — **no** live Ollama required for CI.
- **`ollama` (Track D Slice D3):** [`OllamaModelPipelineAdapter`](../../backend/app/services/model_pipeline_ollama_adapter.py) calls Ollama **`POST /api/generate`** (stdlib **`urllib`**, **`stream: false`**, **`format: json`**). Adapter registry requires non-empty **`GRAPHCLERK_MODEL_PIPELINE_BASE_URL`** and **`GRAPHCLERK_MODEL_PIPELINE_MODEL`** — otherwise **`model_pipeline_ollama_misconfigured`** (**no** silent fallback). Per-purpose enablement is **`model_pipeline_purpose_registry`** (**D4**), not this env alone ([**D2.5**](../decisions/phase_8_model_pipeline_completion_decisions.md)).
- **`openai_compatible`:** still **not implemented** — **`ModelPipelineAdapterNotImplementedError`** (**`model_pipeline_adapter_not_implemented`**).
...
- **Purpose registry (Track D Slice D4 + D6 settings merge):** [`model_pipeline_purpose_registry.py`](../../backend/app/services/model_pipeline_purpose_registry.py) — **`build_default_model_pipeline_purpose_registry(settings)`** disables **all** purposes unless **`GRAPHCLERK_MODEL_PIPELINE_EVIDENCE_ENRICHER_ENABLED=true`**, which enables **`evidence_candidate_enricher`** from **`GRAPHCLERK_MODEL_PIPELINE_EVIDENCE_ENRICHER_MODEL`** (+ optional purpose timeout). Phase 1–8 policy: **only** that role may be enabled (**`adapter` must be `ollama`**); others remain **`model_pipeline_purpose_policy_blocked`**. **No** HTTP inside the registry module. Tests: [`test_phase8_model_pipeline_purpose_registry.py`](../../backend/tests/test_phase8_model_pipeline_purpose_registry.py).
- **Tests (adapter stack):** [`test_phase8_model_pipeline_registry.py`](../../backend/tests/test_phase8_model_pipeline_registry.py), [`test_phase8_model_pipeline_ollama_adapter.py`](../../backend/tests/test_phase8_model_pipeline_ollama_adapter.py), config in [`test_config.py`](../../backend/tests/test_config.py). Optional real-Ollama smoke is **not** part of default CI; gate any future smoke behind **`RUN_INTEGRATION_TESTS=1`** + explicit env if added later.
- **Metadata enrichment orchestration (Track D Slice D5):** [`model_pipeline_metadata_enrichment_service.py`](../../backend/app/services/model_pipeline_metadata_enrichment_service.py) — **`ModelPipelineMetadataEnrichmentService`** composes adapter + **`ModelPipelineOutputValidationService`** + **`ModelPipelineCandidateMetadataProjectionService`**. Tests [`test_phase8_model_pipeline_metadata_enrichment_service.py`](../../backend/tests/test_phase8_model_pipeline_metadata_enrichment_service.py) use **`DeterministicTestModelPipelineAdapter`** / stubs only — **no** network, **no** **`POST /answer`** fields.
- **Ingest wiring (Track D Slice D6):** When **`GRAPHCLERK_MODEL_PIPELINE_EVIDENCE_ENRICHER_ENABLED=true`**, **`build_evidence_enrichment_service`** ([`artifacts.py`](../../backend/app/api/routes/artifacts.py)) composes **`ModelPipelineMetadataEnrichmentService`** after optional language detection; **`EvidenceEnrichmentService`** runs language **then** model metadata enrichment before **`EvidenceUnitService.create_from_candidates`**. Requires **`GRAPHCLERK_MODEL_PIPELINE_ADAPTER=ollama`**, non-empty **`GRAPHCLERK_MODEL_PIPELINE_BASE_URL`**, and non-empty **`GRAPHCLERK_MODEL_PIPELINE_EVIDENCE_ENRICHER_MODEL`** — validated at **`Settings`** parse (**fail loud**). Runtime adapter / validation failures **do not** abort ingestion (no **`graphclerk_model_pipeline`** merge). Tests: [`test_phase8_model_pipeline_ingestion_wiring.py`](../../backend/tests/test_phase8_model_pipeline_ingestion_wiring.py) monkeypatch **`build_evidence_enricher_model_pipeline_adapter`** — **no** live HTTP, **no** **`POST /answer`**.
- **Operator UI visibility (Track D Slice D7a):** Read-only **Phase 8** notes + conditional **`graphclerk_model_pipeline`** readout in **`frontend/src/components/ArtifactsExplorer.tsx`** (and introductory copy on **`EvaluationDashboard.tsx`**) — **no** writable selector, **no** admin config persistence; **no** ingest/retrieve behavior change.
- **Read-only config API + UI (Track D Slice D7b):** **`GET /model-pipeline/config`** ([`model_pipeline.py`](../../backend/app/api/routes/model_pipeline.py)) returns env-derived adapter flags, purpose registry status, and warnings — **no** **`GRAPHCLERK_MODEL_PIPELINE_API_KEY`** exposure, **no** raw **`GRAPHCLERK_MODEL_PIPELINE_BASE_URL`**, **no** outbound model/Ollama calls. **`EvaluationDashboard`** shows the payload read-only. **No** **`POST`/`PUT`/`PATCH`** config routes. Tests: [`test_phase8_model_pipeline_config_api.py`](../../backend/tests/test_phase8_model_pipeline_config_api.py). Verify frontend with **`npm run build`** from **`frontend/`**.
