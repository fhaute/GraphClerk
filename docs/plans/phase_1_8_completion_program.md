...
| **Current state** | **8A–8F** shipped; **8G** **design-only**; **D2** settings + adapter registry; **D3** **`OllamaModelPipelineAdapter`**; **D4** **`model_pipeline_purpose_registry`** (typed per-**`ModelPipelineRole`** policy, **no** HTTP) — **not** wired to ingestion/retrieve. Projection **not** wired to enrichment/ingestion. Audited **`pass_with_notes`**. Decisions: **[`phase_8_model_pipeline_completion_decisions.md`](../decisions/phase_8_model_pipeline_completion_decisions.md)**. |
| **Missing work** | **Track D D5+**: **metadata-only** enrichment orchestration (**D5**); ingestion merge behind **per-purpose** config (**D6**); frontend / operator visibility + selector (**D7**); optional **OpenAI-compatible** adapter (**D3b**); **Phase 8 full-completion audit** (**D8**). |
...
- **Recommended next Track D slice:** **D5** — model metadata enrichment orchestration (**no** ingestion wiring).

**Track D** is **not** complete — **D1**–**D4** (incl. **D2.5**) closed; **D5** is next.
