...
| **Current state** | Enrichment default **no-op**; **Track C4** optional `LanguageDetectionService` injection into `EvidenceEnrichmentService`; **Track C5** persists **`graphclerk_language_aggregation`** on `Artifact.metadata_json` after EU create (text + multimodal ingest); **Track C6** regression tests cover **`language_context`** from selected EU metadata vs artifact aggregation and gated retrieve; **Track C7** UI surfaces **language_context** vs **artifact** `graphclerk_language_aggregation` (Artifacts explorer readout + packet copy; onboarding); **`LanguageDetectionService`** **NotConfigured** default + optional **Lingua** when `GRAPHCLERK_LANGUAGE_DETECTION_ADAPTER=lingua` + **`language-detector`** extra (**Track C3**); `language_context` from **selected** evidence `metadata_json` only; `actor_context` **recording only** — **no** boost. Audited **`pass_with_notes`** — baseline accepted. **No** automatic on-ingest detection in the **default** app wiring. **Slice 7I** boosting **not** implemented. |
...
| **Proposed slices** | **C1** ✅ · **C2** ✅ · **C3** ✅ optional **Lingua** adapter + settings + tests ([`TESTING_RULES.md`](../governance/TESTING_RULES.md)) — [`phase_7_language_detector_dependency_decision.md`](../decisions/phase_7_language_detector_dependency_decision.md) addendum · **C4** ✅ ingestion enrichment wiring (inject `LanguageDetectionService`; default remains no-op) · **C5** ✅ artifact language aggregation persistence (post–`create_from_candidates`) · **C6** ✅ packet/context tests (language_context from selected EU metadata; no production code change) · **C7** ✅ UI/operator visibility (artifact aggregation readout + packet/playground copy; docs) · **C8** Final Phase 7 full-completion audit · optional **C-boost-1** / **C-translation-1** only after explicit approval |
