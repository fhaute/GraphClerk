...
| **Proposed slices** | **C1** ✅ · **C2** ✅ · **C3** ✅ optional **Lingua** adapter + settings + tests ([`TESTING_RULES.md`](../governance/TESTING_RULES.md)) — [`phase_7_language_detector_dependency_decision.md`](../decisions/phase_7_language_detector_dependency_decision.md) addendum · **C4** ✅ ingestion enrichment wiring (inject `LanguageDetectionService`; default remains no-op) · **C5** ✅ artifact language aggregation persistence (post–`create_from_candidates`) · **C6** Packet/context tests with persisted metadata · **C7** UI/operator visibility (if needed) · **C8** Final Phase 7 full-completion audit · optional **C-boost-1** / **C-translation-1** only after explicit approval |
...
- **Recommended next Track C slice:** **C6** — packet/context tests with persisted metadata (if still needed).

**Track C** is **not** complete — **C1** through **C5** are closed; **C6** is next.
