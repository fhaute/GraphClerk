...
- **Recommended next program track:** proceed per **Track D–H** / phase roadmap — **Track C** closure applies to **agreed Phase 7 Phase 1–8 scope** only.
...
| **Current state** | **8A–8F** shipped; **8G** **design-only**; **D2**–**D6** shipped: optional **`evidence_candidate_enricher`** ingest path behind **`GRAPHCLERK_MODEL_PIPELINE_EVIDENCE_ENRICHER_*`** (**Option A** env); **`EvidenceEnrichmentService`** runs language (**Track C**) then **`ModelPipelineMetadataEnrichmentService`** before EU creation; persists **`metadata_json["graphclerk_model_pipeline"]`** only; **no** **`POST /retrieve`** / FileClerk wiring. Audited **`pass_with_notes`** (historical baseline). Decisions: **[`phase_8_model_pipeline_completion_decisions.md`](../decisions/phase_8_model_pipeline_completion_decisions.md)**. |
| **Missing work** | **D7b** optional — writable selector / admin persistence (blocked on persistence + **auth** decision); **D8** Phase 8 **full-completion audit** (Track **D** not closed until audit); optional **D3b** OpenAI-compatible adapter. **D7a** ✅ read-only visibility (frontend + docs). |
...
| **Proposed slices** | **D1** ✅ decisions · **D2** ✅ settings + static registry · **D2.5** ✅ purpose design · **D3** ✅ Ollama HTTP + mocks · **D4** ✅ purpose registry contracts ([`model_pipeline_purpose_registry.py`](../../backend/app/services/model_pipeline_purpose_registry.py)) · **D5** ✅ metadata enrichment orchestration ([`model_pipeline_metadata_enrichment_service.py`](../../backend/app/services/model_pipeline_metadata_enrichment_service.py)) · **D6** ✅ ingest merge (**`evidence_candidate_enricher`** env only; [`artifacts.py`](../../backend/app/api/routes/artifacts.py) + [`evidence_enrichment_service.py`](../../backend/app/services/evidence_enrichment_service.py)) · **D7a** ✅ read-only operator visibility · **D7b** writable selector (**future / optional**) · **D8** Phase 8 **full-completion audit** · optional **D3b** OpenAI-compatible |
...
- **Outcome:** [`frontend/src/components/ArtifactsExplorer.tsx`](../../frontend/src/components/ArtifactsExplorer.tsx) — Phase **8** operator panel (env + EU vs artifact distinction); expanded evidence detail shows **`metadata_json` (raw)** and a readable **`graphclerk_model_pipeline`** subtree **when** the JSON response includes **`metadata_json`** (current **`GET /evidence-units/{id}`** schema may omit it). Optional static note in [`frontend/src/components/EvaluationDashboard.tsx`](../../frontend/src/components/EvaluationDashboard.tsx). Docs: [`GRAPHCLERK_PIPELINE_GUIDE.md`](../onboarding/GRAPHCLERK_PIPELINE_GUIDE.md), [`GRAPHCLERK_ARCHITECTURE.md`](../onboarding/GRAPHCLERK_ARCHITECTURE.md), [`TROUBLESHOOTING_AND_OPERATIONS.md`](../onboarding/TROUBLESHOOTING_AND_OPERATIONS.md), [`EXAMPLES_COOKBOOK.md`](../onboarding/EXAMPLES_COOKBOOK.md), [`TESTING_RULES.md`](../governance/TESTING_RULES.md), [`KNOWN_GAPS.md`](../status/KNOWN_GAPS.md), decision doc **D7a** note. **No** backend, **no** writable selector (**D7b** remains future).
- **Recommended next Track D slice:** **D8** Phase **8** full-completion audit — unless product mandates **D7b** (writable selector) first per Fred; **D7b** stays **optional / pending** until persistence + **auth**.

**Track D** is **not** complete — **D1**–**D7a** (incl. **D2.5**) closed; **D7b** optional; **D8** next for agreed Track **D** closure.
