...
| **Current state** | **8A–8F** shipped; **8G** **design-only**; **D2** settings + adapter registry; **D3** **`OllamaModelPipelineAdapter`**; **D4** **`model_pipeline_purpose_registry`**; **D5** **`ModelPipelineMetadataEnrichmentService`** (adapter → validate → project → merge **`graphclerk_model_pipeline`** on **`EvidenceUnitCandidate.metadata`** only) — **not** wired to ingestion/retrieve/enrichment. Audited **`pass_with_notes`**. Decisions: **[`phase_8_model_pipeline_completion_decisions.md`](../decisions/phase_8_model_pipeline_completion_decisions.md)**. |
| **Missing work** | **Track D D6+**: ingestion merge behind **per-purpose** config (**D6**); frontend / operator visibility + selector (**D7**); optional **OpenAI-compatible** adapter (**D3b**); **Phase 8 full-completion audit** (**D8**). |
...
| **Proposed slices** | **D1** ✅ decisions · **D2** ✅ settings + static registry · **D2.5** ✅ purpose design · **D3** ✅ Ollama HTTP + mocks · **D4** ✅ purpose registry contracts ([`model_pipeline_purpose_registry.py`](../../backend/app/services/model_pipeline_purpose_registry.py)) · **D5** ✅ metadata enrichment orchestration ([`model_pipeline_metadata_enrichment_service.py`](../../backend/app/services/model_pipeline_metadata_enrichment_service.py); **no** ingest wiring) · **D6** Ingestion merge behind explicit **per-purpose** config · **D7** Frontend / operator visibility (**D7a**/**D7b**) · **D8** Phase 8 **full-completion audit** · optional **D3b** OpenAI-compatible |
