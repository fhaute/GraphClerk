...
| **Phase 8** ([`PHASE_8_AUDIT.md`](../audits/PHASE_8_AUDIT.md)) | Contracts, validation, standalone projection, **8G design-only** | **No** production HTTP adapter yet; **D2** settings + **adapter-key** static registry; **D2.5** per-purpose mapping + selector **design only** ([`phase_8_model_pipeline_completion_decisions.md`](../decisions/phase_8_model_pipeline_completion_decisions.md)); **no** merge into ingestion/enrichment as shipped behavior |
...
| **Current state** | **8A–8F** shipped; **8G** **design-only**; **no** production HTTP adapter in shipped paths; **Track D D2** settings + static registry (**`not_configured`** default; reserved adapters fail at registry build); projection **not** wired to enrichment/ingestion. Audited **`pass_with_notes`**. **Track D Slice D1** records completion-path decisions — **[`docs/decisions/phase_8_model_pipeline_completion_decisions.md`](../decisions/phase_8_model_pipeline_completion_decisions.md)** (**design only**). |
| **Missing work** | **Track D D3+**: **Ollama HTTP** adapter + mocks (**D3**); **backend purpose registry / config contracts** (**D4**); **metadata-only** enrichment orchestration (**D5**); ingestion merge behind **per-purpose** config (**D6**); frontend / operator visibility + future selector (**D7**, split **D7a/D7b** if persistence lags); optional **OpenAI-compatible** adapter wave; **Phase 8 full-completion audit** (**D8**). |
...
| **Proposed slices** | **D1** ✅ decisions · **D2** ✅ settings + static registry · **D2.5** ✅ purpose registry + selector **design** (same decision doc) · **D3** Ollama HTTP + mocked tests (explicit model per call; **not** final product-only env shape) · **D4** Backend purpose registry / config contracts · **D5** Model metadata enrichment orchestration (**no** ingest wiring yet) · **D6** Ingestion merge behind explicit **per-purpose** config · **D7** Frontend / operator visibility (optional **D7a** read-only · **D7b** writable after persistence/auth) · **D8** Phase 8 **full-completion audit** · optional **D3b/D4b** OpenAI-compatible |
...
- **Recommended next Track D slice:** **D3** — Ollama HTTP adapter + mocked tests (per **D2.5** — do not freeze single global env model as final product shape).

**Track D** is **not** complete — **D1**, **D2**, and **D2.5** closed; **D3** is next.
