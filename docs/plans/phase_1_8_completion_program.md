...
| **Phase 8** ([`PHASE_8_AUDIT.md`](../audits/PHASE_8_AUDIT.md)) | Contracts, validation, standalone projection, **8G design-only** | **D2** registry/settings; **D3** Ollama adapter (**not** ingest-wired); **D2.5** per-purpose design; **no** merge into ingestion/enrichment as shipped behavior |
...
| **Current state** | **8A–8F** shipped; **8G** **design-only**; **Track D D2** settings + static registry; **Track D D3** **`OllamaModelPipelineAdapter`** (`/api/generate`, stdlib HTTP, mocked tests) when **`ollama`** + **`BASE_URL`** + **`MODEL`** — **not** wired to ingestion/retrieve. Projection **not** wired to enrichment/ingestion. Audited **`pass_with_notes`**. Decisions: **[`phase_8_model_pipeline_completion_decisions.md`](../decisions/phase_8_model_pipeline_completion_decisions.md)**. |
| **Missing work** | **Track D D4+**: **backend purpose registry / config contracts** (**D4**); **metadata-only** enrichment orchestration (**D5**); ingestion merge behind **per-purpose** config (**D6**); frontend / operator visibility + selector (**D7**); optional **OpenAI-compatible** adapter (**D3b**); **Phase 8 full-completion audit** (**D8**). |
...
| **Proposed slices** | **D1** ✅ decisions · **D2** ✅ settings + static registry · **D2.5** ✅ purpose registry + selector **design** · **D3** ✅ Ollama HTTP + mocked tests ([`model_pipeline_ollama_adapter.py`](../../backend/app/services/model_pipeline_ollama_adapter.py); interim env model per **D2.5** Option **B**) · **D4** Backend purpose registry / config contracts · **D5** Model metadata enrichment orchestration (**no** ingest wiring yet) · **D6** Ingestion merge behind explicit **per-purpose** config · **D7** Frontend / operator visibility (**D7a**/**D7b**) · **D8** Phase 8 **full-completion audit** · optional **D3b** OpenAI-compatible |
...
- **Outcome:** [`backend/app/services/model_pipeline_ollama_adapter.py`](../../backend/app/services/model_pipeline_ollama_adapter.py) — **`OllamaModelPipelineAdapter`**, **`POST /api/generate`**, stdlib **`urllib`**, **`format: json`**, error codes (**`model_pipeline_ollama_unavailable`**, **`model_pipeline_ollama_http_error`**, **`model_pipeline_invalid_json`**, **`model_pipeline_schema_mismatch`**). Registry builds Ollama adapter when **`BASE_URL`** + **`MODEL`** set; **`model_pipeline_ollama_misconfigured`** if missing. **No** ingestion wiring, **no** purpose registry, **no** new dependencies. Tests: [`test_phase8_model_pipeline_ollama_adapter.py`](../../backend/tests/test_phase8_model_pipeline_ollama_adapter.py), updated registry tests.
- **Recommended next Track D slice:** **D4** — backend purpose registry / config contracts.

**Track D** is **not** complete — **D1**–**D3** (incl. **D2.5**) closed; **D4** is next.
