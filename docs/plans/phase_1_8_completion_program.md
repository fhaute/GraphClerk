...
| **Current state** | **8A–8F** shipped; **8G** **design-only**; **D2**–**D6** shipped: optional **`evidence_candidate_enricher`** ingest path behind **`GRAPHCLERK_MODEL_PIPELINE_EVIDENCE_ENRICHER_*`** (**Option A** env); **`EvidenceEnrichmentService`** runs language (**Track C**) then **`ModelPipelineMetadataEnrichmentService`** before EU creation; persists **`metadata_json["graphclerk_model_pipeline"]`** only; **no** **`POST /retrieve`** / FileClerk wiring. **D7b** read-only **`GET /model-pipeline/config`** + Evaluation dashboard config table (**no** writes). Audited **`pass_with_notes`** (historical baseline). Decisions: **[`phase_8_model_pipeline_completion_decisions.md`](../decisions/phase_8_model_pipeline_completion_decisions.md)**. |
| **Missing work** | **D7c** optional — writable selector / admin persistence / **auth** (blocked on product decision); **D8** Phase 8 **full-completion audit** (Track **D** not closed until audit); optional **D3b** OpenAI-compatible adapter. **D7a** ✅ EU metadata visibility · **D7b** ✅ read-only **`GET /model-pipeline/config`** + dashboard table (**no** writes). |
...
- **Recommended next Track D slice:** **D8** Phase **8** full-completion audit when agreed — **D7c** writable selector remains **optional / pending** (persistence + **auth**).

**Track D** is **not** complete — **D1**–**D7b** (incl. **D2.5**) closed; **D7c** optional; **D8** next for agreed Track **D** closure.
