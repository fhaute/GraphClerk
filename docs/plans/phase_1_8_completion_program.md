...
| **Current state** | Enrichment default **no-op**; **Track C4** allows callers to inject `LanguageDetectionService` into `EvidenceEnrichmentService` so ingest can merge **language routing metadata** onto `EvidenceUnitCandidate` before persistence — **no** automatic API/route wiring of Lingua; `LanguageDetectionService` **NotConfigured** default + optional **Lingua** when `GRAPHCLERK_LANGUAGE_DETECTION_ADAPTER=lingua` + **`language-detector`** extra (**Track C3**); aggregation helper **pure**; `language_context` from **selected** evidence `metadata_json` only; `actor_context` **recording only** — **no** boost. Audited **`pass_with_notes`** — baseline accepted. **No** automatic on-ingest detection in the **default** app wiring. **Slice 7I** boosting **not** implemented. |
...
- **Recommended next Track C slice:** **C5** — artifact language aggregation persistence.

**Track C** is **not** complete — **C1**, **C2**, **C3**, and **C4** are closed; **C5** is next.
