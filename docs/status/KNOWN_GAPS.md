...
- **Phase 8 audit**: **`pass_with_notes`** — [`docs/audits/PHASE_8_AUDIT.md`](../audits/PHASE_8_AUDIT.md) (**2026-05-03**); accepts baseline only — ingestion merge, purpose registry, and operator UI remain gaps beyond **D2** settings/registry + **D3** Ollama adapter module.
- **Inference path**: **`NotConfigured`** remains the **default** at registry build. **`ollama`** builds [`OllamaModelPipelineAdapter`](../../backend/app/services/model_pipeline_ollama_adapter.py) when **`BASE_URL`** + **`MODEL`** are set (**Track D3**); **`openai_compatible`** remains **not implemented** (**D3b**). Stdlib HTTP only; **no** vLLM/OpenAI client yet. **8G** north-star remains broader than shipped wiring.
- **Settings + registry (D2/D3)** (`GRAPHCLERK_MODEL_PIPELINE_*`, [`model_pipeline_registry.py`](../../backend/app/services/model_pipeline_registry.py)): **no** **per-purpose** registry (**D4**); registry is **not** wired into ingestion or **`POST /retrieve`** — enabling **`ollama`** does **not** run model calls on ingest/retrieve paths until later slices.
