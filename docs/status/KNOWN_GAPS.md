...
- **`LanguageDetectionService`**: adapter shell plus optional **Lingua** adapter behind `GRAPHCLERK_LANGUAGE_DETECTION_ADAPTER=lingua` (requires **`language-detector`** extra); **default remains `not_configured`**. **Track C4:** language detection can be **injected** into `EvidenceEnrichmentService` so ingest merges language **metadata** onto candidates before EU persistence; the **default** API/install path still performs **no** automatic detection unless a caller wires enrichment with a detector (routes/settings are **not** changed to auto-enable Lingua by default). **No** “always-on” remote detector.
