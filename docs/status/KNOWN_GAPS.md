...
- **Phase 8 audit**: **`pass_with_notes`** — [`docs/audits/PHASE_8_AUDIT.md`](../audits/PHASE_8_AUDIT.md) (**2026-05-03**); accepts baseline only — **D6** ingestion merge, operator UI, and full-completion audit remain gaps beyond shipped contracts + **D5** orchestration.
- **Inference path**: **`NotConfigured`** remains the **default** at adapter registry build. **`ollama`** builds [`OllamaModelPipelineAdapter`](../../backend/app/services/model_pipeline_ollama_adapter.py) when **`BASE_URL`** + **`MODEL`** are set (**D3**); **`openai_compatible`** remains **not implemented** (**D3b**). Stdlib HTTP only; **no** vLLM/OpenAI client yet. **8G** north-star remains broader than shipped wiring.
- **Purpose registry (D4)** ([`model_pipeline_purpose_registry.py`](../../backend/app/services/model_pipeline_purpose_registry.py)): typed config + policy — **all** purposes **disabled** by factory default; **no** automatic model calls; **not** wired into ingestion or **`POST /retrieve`**. Global **`ollama`** adapter env still does **not** imply per-purpose runs until **D6** merges resolution into ingest paths.
- **`ModelPipelineMetadataEnrichmentService` (D5)** ([`model_pipeline_metadata_enrichment_service.py`](../../backend/app/services/model_pipeline_metadata_enrichment_service.py)): orchestrates adapter → validation → projection → **`EvidenceUnitCandidate.metadata["graphclerk_model_pipeline"]`** merge — **not** wired into text/multimodal ingestion or **`EvidenceEnrichmentService`** (**D6**).
- **`ModelPipelineCandidateMetadataProjectionService`** remains a **pure** projection helper; merge into candidates happens only via **D5** (or future callers), **not** inside ingestion services yet.
- **Model output is not evidence**: pipeline output stays **metadata-only** under **`metadata["graphclerk_model_pipeline"]`** on candidates when attached; **no** automatic **`EvidenceUnit`** / candidate **`text`** / **`source_fidelity`** mutation from model pipeline modules.
