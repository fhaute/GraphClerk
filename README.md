...
- **Phase 6** (**implemented `pass_with_notes`** — `docs/audits/PHASE_6_AUDIT.md`, `docs/phases/graph_clerk_phase_6_productization_ui_evaluation_hardening.md`): React/Vite UI in `frontend/` against **live** APIs; **demo/inspection productization**, **not** production enterprise software. See [Web UI](#web-ui-phase-6). **Onboarding / pipeline integration (living docs):** [`docs/onboarding/README.md`](docs/onboarding/README.md) (index), **[`docs/onboarding/GRAPHCLERK_ARCHITECTURE.md`](docs/onboarding/GRAPHCLERK_ARCHITECTURE.md)** (architecture + Mermaid), **[`docs/onboarding/FEED_CONTENT_MINIMAL_GUIDE.md`](docs/onboarding/FEED_CONTENT_MINIMAL_GUIDE.md)** (minimal hands-on path), **[`docs/onboarding/TROUBLESHOOTING_AND_OPERATIONS.md`](docs/onboarding/TROUBLESHOOTING_AND_OPERATIONS.md)** (failure modes + Qdrant/HTTP runbooks), **[`docs/onboarding/EXAMPLES_COOKBOOK.md`](docs/onboarding/EXAMPLES_COOKBOOK.md)** (PowerShell / curl / Python examples) — plus this README, `docs/api/API_OVERVIEW.md`, `docs/release/RELEASE_CHECKLIST.md`, `docs/evaluation/EVALUATION_METHOD.md`, `docs/demo/PHASE_6_DEMO_CORPUS.md`.
- **Phase 7** (**implemented for the agreed Phase 1–8 completion scope**; full-completion audit **`pass`** — [`docs/audits/PHASE_7_FULL_COMPLETION_AUDIT.md`](docs/audits/PHASE_7_FULL_COMPLETION_AUDIT.md), 2026-05-01 — **Track C Slice C9**): **context intelligence** — optional **Lingua** language detection when **`GRAPHCLERK_LANGUAGE_DETECTION_ADAPTER=lingua`** + **`language-detector`** extra (**default `not_configured`**); **`POST /artifacts`** wires enrichment; EU **`metadata_json`** language keys; **`Artifact.metadata_json.graphclerk_language_aggregation`**; **`RetrievalPacket.language_context`** from **selected** evidence metadata (**not** translation); **`actor_context`** **recording only** (**no** retrieval boosting). **Baseline history:** [`docs/audits/PHASE_7_AUDIT.md`](docs/audits/PHASE_7_AUDIT.md) (`pass_with_notes`, 2026-05-02). **Not** translation; **Slice 7I** boosting **not implemented** (`docs/status/KNOWN_GAPS.md`).
- **Phase 8** (**implemented for the agreed Phase 1–8 completion scope**; full-completion audit **`pass`** — [`docs/audits/PHASE_8_FULL_COMPLETION_AUDIT.md`](docs/audits/PHASE_8_FULL_COMPLETION_AUDIT.md), **2026-05-02** — **Track D Slice D8**): specialized **model pipeline** — contracts/envelopes, **`NotConfigured`** default, **tests-only** deterministic adapter, **`OllamaModelPipelineAdapter`** (stdlib HTTP; mocked tests), static registry + **`GRAPHCLERK_MODEL_PIPELINE_*`** settings, **purpose registry** (**`evidence_candidate_enricher`** only enabled under policy), validate→project orchestration, optional **`POST /artifacts`** metadata-only merge to **`EvidenceUnit.metadata_json["graphclerk_model_pipeline"]`** when explicitly configured, read-only **`GET /model-pipeline/config`**, operator UI (**Artifacts** + **Evaluation**). **Default** install makes **no** model HTTP calls. **Baseline history:** [`docs/audits/PHASE_8_AUDIT.md`](docs/audits/PHASE_8_AUDIT.md) (**`pass_with_notes`**, **2026-05-03**). **Still not implemented:** **`openai_compatible`** adapter; **writable** selector / **config persistence** / **admin-auth**; **`POST /answer`**; **`routing_hint_generator`** enablement; Phase **9**. See [`docs/phases/graph_clerk_phase_8_specialized_model_pipeline.md`](docs/phases/graph_clerk_phase_8_specialized_model_pipeline.md) for broader phase-doc context vs agreed completion scope.
...
- **Translation**, query translation, and ActorContext-driven retrieval boosting (**Slice 7I**) — **not** implemented. Default language detection remains **off** (**`not_configured`**); optional **Lingua** is explicit-operator configuration — see [`docs/audits/PHASE_7_FULL_COMPLETION_AUDIT.md`](docs/audits/PHASE_7_FULL_COMPLETION_AUDIT.md) non-goals.
