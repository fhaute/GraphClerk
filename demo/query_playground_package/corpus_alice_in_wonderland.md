# GraphClerk — Large query-playground demo corpus

This document bundles **Project Gutenberg ebook #11** (*Alice's Adventures in Wonderland*, Lewis Carroll) — **public domain** in the United States.

## Deterministic routing token (copy exactly as your question)

For `GRAPHCLERK_SEMANTIC_SEARCH_EMBEDDING_ADAPTER=deterministic_fake` (local integration-test mode), the companion loader sets the semantic index `embedding_text` to the following **exact** string. After you **backfill** that index into Qdrant, paste **only** this token into the Query playground **Question** field so route selection matches this index:

```text
GRAPHCLERK_LARGE_DEMO_QUERY_PLAYGROUND_V1
```

When you use a **real** embedding model and a **single** freshly indexed semantic index in a clean database, you can instead ask natural-language questions about the chapters below.
